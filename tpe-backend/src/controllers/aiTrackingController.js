    ...
    try {
      ...
      const result = await pool.query(
        `INSERT INTO contractor_engagement_events 
         ...
         RETURNING *`,
        [contractor_id, event_type, safeJsonStringify(event_data), channel, session_id]
      );
        ...
        for (const event of events) {
          const result = await pool.query(
            ...
            [
              contractor_id,
              event.event_type,
              safeJsonStringify(event.event_data || {}),
              event.channel || 'web',
              event.session_id
            ]
          );
  ...
  async updatePreferences(req, res) {
    try {
      ...
      if (communication_preferences) {
        ...
        values.push(safeJsonStringify(communication_preferences));
      }

      if (learning_preferences) {
        updates.push(`learning_preferences = $${++paramCount}`);
        values.push(safeJsonStringify(learning_preferences));
      }

...
/**
 * Local Development Monitoring Dashboard
 * Displays real-time health metrics for state machine and AI concierge (localhost)
 */

const { query } = require('../src/config/database');

async function monitorLocal() {
  console.clear();
  console.log('📊 TPX AI Concierge - LOCAL Development Monitoring\n');
  console.log('='.repeat(60));
  console.log('Environment: LOCAL DEVELOPMENT (localhost:5000)');
  console.log('Database: Local PostgreSQL (tpedb)');
  console.log('='.repeat(60));
  ...
  try {
    ...
    // 5. Development-specific info
    console.log('\n💻 Development Info:');
    console.log(`  Backend URL: http://localhost:5000`);
    console.log(`  Frontend URL: http://localhost:3002`);
    console.log(`  Web Dashboard: http://localhost:3002/admindashboard/monitoring`);
    console.log(`  API Endpoint: http://localhost:5000/api/state-machine/monitoring`);
    ...
    console.log('\n💡 Tip: Make sure your local backend is running on port 5000');
  }
}

// Run monitoring every 10 seconds
setInterval(monitorLocal, 10000);
monitorLocal(); // Initial run
